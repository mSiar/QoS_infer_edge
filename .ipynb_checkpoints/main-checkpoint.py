...
class DecisionMaker_energy_priority_reqFirst:
    def __init__(self):
        pass
    ...
    def decide_allocation(self, request_sets, edge_nodes, models, completed_requests, response_logs, response_logs2, nodes_util, nodes_energy, current_time=None):
        ...
        cur_eng = [nd.current_energy_usage() for nd in (edge_nodes)]
...
class DecisionMaker_minimumCTN:
    def __init__(self):
        pass
        ...
        for req_set in request_sets: 
            ...
            for _ in range(remaining):
                ...
                if req_obj is None:
                    break 
                obj_fun = []
                obj_fun = [[float('inf') for _ in range(len(models))] for _ in range(len(edge_nodes))]
                ...
                for ctn in (moel_container):
                    ...
                    temp_min_time = min_time+real_start
                    temp_max_time = max_time+real_start
                    normal_time = (real_finish-(temp_min_time))/((temp_max_time)-(temp_min_time))
                    normal_eng = (energy_needed-min_eng)/(max_eng-min_eng)
                    obj_fun[node.id][ctn["model"].id] = normal_time+normal_eng

                np_array = np.array(obj_fun)
                check_inf = np.isfinite(np_array).any()
                if check_inf:
                    min_val = float("inf")
                    min_node = -1
                    min_model = -1
    
                    for i, row in enumerate(obj_fun):
                    ...
                    node = next(n for n in edge_nodes if n.id == min_node)  
                    model = next(m for m in models if m.id == min_model)  
                    start, finish, acc_real, E_dyn_real = node.process_request(req_obj.arrival_time, model)
                    ...
                    req_set.mark_request_completed()       
                    chosen_util_increment = node.service_time(model) / TIME_SLOT
                    current_util[node.id] += chosen_util_increment
...
class DecisionMaker_epsilon_greedy_reqFirst:
    def __init__(self):
        pass
    ...
    def decide_allocation(self, request_sets, edge_nodes, models, completed_requests, response_logs, response_logs2, nodes_util, nodes_energy, current_time=None):
        ...
        for req_set in request_sets: 
            ...
            for _ in range(remaining):
                ...
                for ctn in (moel_container):
                    ...
                    if rnd<epsilon:
                        node_index = random.randrange(len(edge_nodes))
                        node = edge_nodes[node_index]
                        model_id = random.randrange(len(models))
                        model = models[model_id]
                        accuracy = model.accuracy
                        energy_needed = node.energy_per_request(model)
                        srv_time = node.service_time(model)
                    else:
                        accuracy = ctn["accuracy"]
                        energy_needed = ctn["dynamic_energy"]
                        srv_time = ctn["service_time"]
                        model = ctn["model"]
                        node_index = ctn["edge_id"]
                        node = next(n for n in edge_nodes if n.id == node_index) 
                    ...
                    util_increment = srv_time/TIME_SLOT 
                    ...
                    qos_accuracy_ok = (round(accuracy + sum_est_acc,
                              max(decimal_places(accuracy), decimal_places(req_set.required_accuracy))) >=
                        round(req_set.required_accuracy * (req_set.num_completed_request + 1),
                              decimal_places(req_set.required_accuracy)))
        ...
        for (arrival, start, finish, node.id, model_name) in completed_requests:
            arrivals_node[node.id][model_name].append(arrival)
...
class DecisionMaker_edgeParams_request_priority_reqFirst_dynamic:
    def __init__(self):
        pass
    def decide_allocation(self, request_sets, edge_nodes, models, completed_requests, response_logs, response_logs2, nodes_util, nodes_energy, current_time=None):
        init_busy = [node.busy_until for node in edge_nodes]
        ...
        for node in edge_nodes:
            for model in models:
                ...
                moel_container.append({
                    ...
                    "energy": energy_metric,     
                    "dynamic_energy": E_dyn,       
                    "waiting_time" : node.busy_until,
                    "energy_util": E_dyn/node.available_energy,
                    "current_util": 0
                })
        ...
        for req_set in request_sets: 
            ...
            for _ in range(remaining):
                ...
                for ctn in (moel_container):
                    node_id = ctn["edge_id"]
                    node = next(n for n in edge_nodes if n.id == node_id)  
                    ctn["waiting_time"]=  node.busy_until+ ctn["service_time"]
                    ctn["energy_util"]= ctn["dynamic_energy"]/node.available_energy
                    ctn["current_util"] = current_util[ctn["edge_id"]]
                moel_container.sort(key=lambda r: (r["waiting_time"], r["current_util"], r["energy_util"], -r["accuracy"]))
                for ctn in (moel_container):
                    node_id = ctn["edge_id"]
                    node = next(n for n in edge_nodes if n.id == node_id)     
                    arriv_time = []
...
class DecisionMaker_epsilonGreedy_Mlfirst:
    def __init__(self):
        pass  
        ...
        for ctn in (moel_container):
            ...
            for r_set in request_sets:
                ...
                for _ in range(remaining):
                    ...
                    real_start = max(node.busy_until, req_obj.arrival_time)
                    real_finish = real_start + exec_time
                    qos_deadline_ok = (round(real_finish, decimal_places(r_set.qos_response_time))
                        <=
                        round(req_obj.arrival_time + r_set.qos_response_time, decimal_places(r_set.qos_response_time)) )
                    ...
                    qos_accuracy_ok = (round(accuracy + sum_est_acc,
                              max(decimal_places(accuracy), decimal_places(r_set.required_accuracy))) >=
                        round(r_set.required_accuracy * (r_set.num_completed_request + 1),
                              decimal_places(r_set.required_accuracy)))
...
def main():
    ...
    random.seed(42)
    np.random.seed(42)
    energy_scale = [random.randint(10,100) for i in range(DEFAULT_EDGE_NUM)]  # changed this
    ...
    time1 = time2 = 0
    time1 = time.time() ;
    sim0 = Simulator(edge_nodes=build_nodes(edge_num, None), models=copy.deepcopy(models), duration=60, decision_maker=DecisionMaker_energy_priority_reqFirst())
    sim0.request_sets = copy.deepcopy(base_requests)
    sim0.run()
    log=[]
    time2 = time.time()
    time_spended = time2-time1
    print("time_spended:  ", time_spended)
    approach = "Energy-aware-FirstFit"
    sim0.print_stats(approach, edge_num, DEFAULT_ARRIVAL, None, log)
    sim0.save_response_logs(approach)
    sim0.save_response_logs_2(approach)
    ...
    sim2=[]
    log=[]
    sim2 = Simulator(edge_nodes=build_nodes(edge_num, None), models=copy.deepcopy(models), duration=60, decision_maker = DecisionMaker_energy_request_priority())
    ...
    sim2.print_stats(approach, edge_num, DEFAULT_ARRIVAL, None, log)
    sim2.save_response_logs(approach)
    sim2.save_response_logs_2(approach)

    time1 = time2 = 0
    time1 = time.time() ;
    sim3 = []
    sim3 = Simulator(edge_nodes=build_nodes(edge_num, None), models=copy.deepcopy(models), duration=60, decision_maker = DecisionMaker_edgeParams_request_priority())
    ...
    time_spended = time2-time1
    print("time_spended:  ", time_spended)
    approach = "Multi-criteria-Greedy"
    ...
    time_spended = time2-time1
    print("time_spended:  ", time_spended)
    approach = "Multi-criteria-FirstFit"
    ...
    time1 = time2 = 0
    time1 = time.time() ;
    sim5 = Simulator(edge_nodes=build_nodes(edge_num, None), models=copy.deepcopy(models), duration=60, decision_maker= DecisionMaker_epsilonGreedy_Mlfirst())
    ...
    time_spended = time2-time1
    print("time_spended:  ", time_spended)
    approach = "EpsilonGreedy-MLfirst"
    sim5.print_stats(approach, edge_num, DEFAULT_ARRIVAL, None, log)
    sim5.save_response_logs(approach)
    sim5.save_response_logs_2(approach)

    time1 = time2 = 0
    time1 = time.time() ;
    sim7 = Simulator(edge_nodes=build_nodes(edge_num, None), models=copy.deepcopy(models), duration=60, decision_maker= DecisionMaker_epsilon_greedy_reqFirst())
    sim7.request_sets = copy.deepcopy(base_requests)
    sim7.run()
    log=[]
    time2 = time.time()
    time_spended = time2-time1
    print("time_spended:  ", time_spended)
    approach = "EpsilonGreedy-Reqfirst"
    sim7.print_stats(approach, edge_num, DEFAULT_ARRIVAL, None, log)
    sim7.save_response_logs(approach)
    sim7.save_response_logs_2(approach)
